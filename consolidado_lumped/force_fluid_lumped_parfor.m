function [Df,V_store_H,Fext] = force_fluid_lumped_parfor(GDof,numberRes,m_index,n_index,nnode,nelem,...
...
parfor uu = 1:nelem
    ...
    %Identifying coordinates of the nodes
    xcoord2 = xnodes(index); %column vector
    ycoord2 = ynodes(index);
...
%     Ffluid2{:,:,index(1)} = Ffluid2{:,:, index(1)} + f_mean;
%     Ffluid(:,:,index(2)) = Ffluid(:,:, index(2)) + f_mean;
%     Ffluid(:,:,index(3)) = Ffluid(:,:, index(3)) + f_mean;
%     Ffluid(:,:,index(4)) = Ffluid(:,:, index(4)) + f_mean;
% 
